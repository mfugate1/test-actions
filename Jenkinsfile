node ('docker') {
    ...
    withCredentials([usernameColonPassword(credentialsId: '506843ca-f776-43d2-aa72-143072aa2688', variable: 'USERPASS')]) {
        String script = "git pull origin ${BRANCH_NAME} && npm version patch -m 'Automated version bump to %s' && git log -1 && git push https://${USERPASS}@github.com/mfugate1/test-actions.git ${BRANCH_NAME}"
